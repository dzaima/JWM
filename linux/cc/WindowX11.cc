...
void WindowX11::setClass(const std::string& name, const std::string& class_) {
    XClassHint *classHint = XAllocClassHint();
    if (classHint) {
        classHint->res_name = const_cast<char*>(name.c_str());
        classHint->res_class = const_cast<char*>(class_.c_str());
...
extern "C" JNIEXPORT void JNICALL Java_io_github_humbleui_jwm_WindowX11__1nSetX11Class
        (JNIEnv* env, jobject obj, jbyteArray name, jbyteArray class_) {
    jwm::WindowX11* instance = reinterpret_cast<jwm::WindowX11*>(jwm::classes::Native::fromJava(env, obj));

    instance->setClass(bytesToString(env, name), bytesToString(env, class_));
}
